...
The debug bridge records native command names, layer membership, pin-tier visibility, and other diagnostics in `data-native-map-bridge-debug-*` attributes on the document element and exposes the same `getDebugState` shape as the app-local native scaffolds, which makes native-provider smoke tests possible before the iOS/Android renderers exist.
...
Both native scaffolds now also record layer membership and pin-tier visibility from `addToLayer`, `addItemsToLayer`, `configurePinTier`, and `setPinTierVisible`, keeping progressive pin ownership inspectable before native renderers are enabled.
...
The Android debug state still tracks native marine tile-overlay definitions, layer membership, pin-tier visibility, and item visibility. Shared XYZ and WMS marine overlay payloads now create native Google `TileOverlay` handles, and marker, circle, polyline, and polygon payloads create native Google Maps handles with provider-scoped `click` events. The iOS scaffold now declares provider-scoped MapKit events and sends `moveend`, `zoomend`, and annotation `click` notifications through Capacitor listeners.
