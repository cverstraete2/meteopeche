...
- Added provider-neutral `markerIds` and `shapeIds` aliases to the iOS MapKit debug state, matching Android Google and the local native debug bridge while preserving MapKit-specific annotation and shape-overlay ids.
...
- Added explicit native event expectation metadata to those smoke routes, including minimum event counts, required provider-scoped move/zoom events, optional click events, and DOM debug datasets for event/camera telemetry.
- Mirrored the smoke route matrix summary into DOM debug attributes, so browser smoke harnesses can verify route count, route labels, route params, route URLs, expected outcomes, native event-route labels, native capability errors, unsupported-route labels, and native debug dataset expectations even when they cannot access app-owned window objects directly.
- Added native capability-failure entries to `mapProviderSmokeRoutes()` for both `apple-native` and `google-native`, covering protocol mismatch, missing commands, and missing provider-scoped events with expected capability-error metadata.
- Exposed normalized smoke-route params, URLs, expected outcomes, native capability errors, native debug dataset expectations, and native unsupported-route labels in DOM/debug metadata, completing direct browser visibility for ready, event, not-ready, and capability-failure route categories.
