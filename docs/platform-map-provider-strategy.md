...
- Mirrored the smoke route matrix summary into DOM debug attributes, so browser smoke harnesses can verify route count, route labels, and native event-route labels even when they cannot access app-owned window objects directly.
...
- Exposed normalized smoke-route params and native unsupported-route labels in DOM/debug metadata, completing direct browser visibility for ready, event, not-ready, and capability-failure route categories.
