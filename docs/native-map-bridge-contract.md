...
Because some browser smoke harnesses cannot read app-owned window objects directly, the debug inspector also mirrors route metadata into `data-map-provider-smoke-route-count`, `data-map-provider-smoke-route-labels`, `data-map-provider-smoke-event-route-labels`, and `data-map-provider-smoke-capability-route-labels` on the document element.
