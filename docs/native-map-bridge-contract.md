...
- HTML-backed icons are passed as `{ className, html, iconSize, iconAnchor, tooltipAnchor, popupAnchor }`. Native implementations can render a simplified equivalent, but should keep the anchor, tooltip anchor, opacity, interactivity, and z-index behavior where the platform exposes those controls. Debug state reports marker anchor, tooltip-anchor, opacity, and z-index payload counts for native parity checks.
