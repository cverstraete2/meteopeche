...
Because some browser smoke harnesses cannot read app-owned window objects directly, the debug inspector also mirrors route metadata into `data-map-provider-smoke-route-count`, `data-map-provider-smoke-route-labels`, `data-map-provider-smoke-expected-outcomes`, `data-map-provider-smoke-event-route-labels`, `data-map-provider-smoke-capability-route-labels`, `data-map-provider-smoke-native-capability-errors`, `data-map-provider-smoke-unsupported-route-labels`, and `data-map-provider-smoke-native-debug-datasets` on the document element. Each route object also exposes normalized `params` beside its URL and expected outcome.
