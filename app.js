...
const MOBILE_VIEWS = ["map", "today", "weather", "activity", "journal", "more"];
...
const els = {
  ...
  todayDepthControl: document.querySelector(".today-depth-control"),
  todayDepthButton: document.querySelector("#todayDepthButton"),
  todayDepthPicker: document.querySelector("#todayDepthPicker"),
  ...
  todaySelectedSummary: document.querySelector("#todaySelectedSummary"),
  todaySeriesLegend: document.querySelector("#todaySeriesLegend"),
...
function restoreState() {
  ...
  if (requestedView) state.activeMobileView = normalizeMobileView(requestedView);
...
function maybeShowOnboarding() {
  if (new URLSearchParams(window.location.search).get("skipOnboarding") === "1") {
...
function bindEvents() {
  ...
  els.todayCurveCanvas?.addEventListener("pointermove", handleTodayCurvePointer);
  ...
  els.todayCompassCanvas?.addEventListener("click", handleTodayCompassClick);
...
function setTodayDepthValue(value) {
  const depthValue = clamp(Math.round(Number(value) || 0), 0, 80);
  if (depthValue === state.depth) {
    renderTodayDepthWheel(depthValue);
    return;
  }
  updateDepth(depthValue);
  renderTodayView(getSelectedDay());
  renderMarineOverlay();
}
...
function selectTodayDepthWheelItem(item) {
  ...
  setTodayDepthValue(state.depth + offset);
}

function handleTodayDepthWheel(event) {
  if (!state.todayDepthPickerOpen) return;
  event.preventDefault();
  event.stopPropagation();
  const step = event.deltaY > 0 ? 1 : -1;
  setTodayDepthValue(state.depth + step);
}

function handleTodayDepthPointerDown(event) {
  if (!state.todayDepthPickerOpen) return;
  event.stopPropagation();
  ...
  els.todayDepthPicker?.setPointerCapture?.(event.pointerId);
}

function handleTodayDepthPointerMove(event) {
  ...
  const nextDepth = drag.startDepth + Math.round(delta / 32);
  setTodayDepthValue(nextDepth);
}

function handleTodayDepthPointerRelease(event) {
  const drag = state.todayDepthDrag;
  if (!drag || drag.pointerId !== event.pointerId) return;
  event.stopPropagation();
  els.todayDepthPicker?.releasePointerCapture?.(event.pointerId);
  window.setTimeout(() => {
...
function todayFocusDefinitions(sample = {}, day = {}) {
  return [
    ...
    },
    {
      key: "wind",
      label: "Vent",
      value: sample.windSpeed ?? day?.windAvg,
      unit: "kt",
      decimals: 0,
    ...
    },
    {
      key: "wave",
      label: "Houle",
      value: sample.waveHeight ?? day?.waveAvg,
      unit: "m",
      decimals: 1,
...
function todayFocusDetail(sample, day) {
  if (!day) return "--";
  const direction = sample.depthDirection ?? sample.currentDirection ?? day.depthDirection ?? day.surfaceCurrentDirection;
  const gust = sample.windGust ?? day.windGust;
  const parts = [
    isValidNumber(direction) ? compassLabel(direction) : null,
    isValidNumber(gust) ? `rafales ${formatNumber(gust, 0)} kt` : null,
    sample.depthSource === "copernicus" ? "Copernicus" : null,
  ].filter(Boolean);
  return parts.join(" · ") || "--";
}

function todaySelectedSummary(sample, day) {
  if (!day) return "--";
  return [
    `courant ${formatForceValue((sample.depthCurrent ?? day.depthCurrent) ?? (sample.surfaceCurrent ?? day.surfaceCurrent), "kt", 2)}`,
    `houle ${formatForceValue(sample.waveHeight ?? day.waveAvg, "m", 1)}`,
    `rafales ${formatForceValue(sample.windGust ?? day.windGust, "kt", 0)}`,
  ].join(" · ");
}

function formatForceValue(value, unit, decimals) {
  return isValidNumber(value) ? `${formatNumber(value, decimals)} ${unit}` : "--";
}

function renderTodayLegend() {
  if (!els.todaySeriesLegend) return;
  const items = [
    ["Courant", themeColor("current")],
    ["Fond", themeColor("depth")],
    ["Houle", themeColor("wave")],
    ["Rafales", themeColor("gust")],
  ];
  els.todaySeriesLegend.innerHTML = items.map(([label, color]) => `
    <span><i style="--series-color:${escapeHtml(color)}"></i>${escapeHtml(label)}</span>
  `).join("");
}

function drawTodayCompass(day, sample = {}) {
  const canvas = els.todayCompassCanvas;
  if (!canvas) return;
  ...
  ctx.strokeStyle = "rgba(44, 77, 112, 0.20)";
  ctx.lineWidth = 1.5;
  ...
  for (let deg = 0; deg < 360; deg += 10) {
    ...
    ctx.strokeStyle = deg % 90 === 0 ? "rgba(7, 20, 42, 0.34)" : "rgba(44, 77, 112, 0.18)";
    ctx.lineWidth = deg % 90 === 0 ? 2.5 : 1.5;
    ctx.beginPath();
  ...
  [["N", 0], ["E", 90], ["S", 180], ["O", 270]].forEach(([label, deg]) => {
    const point = polar(deg, radius * 0.84);
...
function drawTodayCompassFocusArrow(ctx, cx, cy, length, direction, color) {
  ...
  const tip = { x: cx + ux * length, y: cy + uy * length };
  ...
  const neckLeft = { x: cx + ux * bodyEndDistance + px * bodyHalf, y: cy + uy * bodyEndDistance + py * bodyHalf };
  const neckRight = { x: cx + ux * bodyEndDistance - px * bodyHalf, y: cy + uy * bodyEndDistance - py * bodyHalf };
  const shoulderLeft = { x: cx + ux * bodyEndDistance + px * headHalf, y: cy + uy * bodyEndDistance + py * headHalf };
  const shoulderRight = { x: cx + ux * bodyEndDistance - px * headHalf, y: cy + uy * bodyEndDistance - py * headHalf };
  ...
  ctx.quadraticCurveTo(baseRound.x + px * baseHalf, baseRound.y + py * baseHalf, baseRound.x, baseRound.y);
  ctx.quadraticCurveTo(baseRound.x - px * baseHalf, baseRound.y - py * baseHalf, bodyRight.x, bodyRight.y);
...
function handleTodayCompassClick(event) {
  const day = getSelectedDay();
  if (!day || !els.todayCompassCanvas) return;
  const rect = els.todayCompassCanvas.getBoundingClientRect();
  ...
  if (distance < Math.min(rect.width, rect.height) * 0.18) {
    const activeIndex = Math.max(0, definitions.findIndex((definition) => definition.key === state.todayFocusMode));
    state.todayFocusMode = definitions[(activeIndex + 1) % definitions.length].key;
  } else {
    const angle = normalizeDirection((Math.atan2(x, -y) * 180) / Math.PI);
    const nearest = definitions
      .filter((definition) => isValidNumber(definition.direction))
      .map((definition) => ({
        definition,
        delta: directionDelta(angle, definition.direction),
      }))
      .sort((a, b) => a.delta - b.delta)[0]?.definition;
    state.todayFocusMode = nearest?.key ?? definitions[0].key;
  }
...
function directionDelta(a, b) {
  const diff = Math.abs(normalizeDirection(a) - normalizeDirection(b));
  return Math.min(diff, 360 - diff);
}

function drawTodayCurve(day) {
  const canvas = els.todayCurveCanvas;
  if (!canvas) return;
  const rect = canvas.getBoundingClientRect();
  if (rect.width < 2 || rect.height < 2) return;
  const ctx = setupCanvas(canvas);
...
function handleTodayCurvePointer(event) {
  if (!els.todayCurveCanvas || (event.type === "pointermove" && event.buttons !== 1)) return;
  event.preventDefault();
  const rect = els.todayCurveCanvas.getBoundingClientRect();

  if (event.pointerId != null && event.type === "pointerdown") {
    els.todayCurveCanvas.setPointerCapture?.(event.pointerId);
    state.todayCurveDrag = {
      pointerId: event.pointerId,
      startX: event.clientX,
      startMinute: selectedTimelineMinute(),
      dragged: false,
    };
    return;
  }
...
function playTodayTimeline() {
  const startHour = Math.round(selectedTimelineMinute() / 60);
  let hour = startHour;
  const tick = () => {
    hour = (hour + 1) % 24;
    state.timelineMinute = hour * 60;
    renderTodayView(getSelectedDay());
    renderDayTimeline(getSelectedDay());
    if (hour !== startHour) setTimeout(tick, 120);
  };
  tick();
}
